...
class GroupCoordinator:
    ...
    def __init__(
        ...
        # device-bound (see init_distributed_environment). TP/DP/PP subgroups are
        # carved from it with split_group, which requires the parent communicator
        # to be initialized/bound up front; each device subgroup retains the
        # parent's "cuda:nccl2" backend, and the cpu subgroup additionally keeps
        # "cpu:gloo" for direct CPU-side coordination. Non-nccl2 paths (e.g. a
        # plain gloo or NPU/XPU run) fall back to the upstream new_group path.
        is_mooncake = "mooncake" in torch_distributed_backend
        use_split_group = is_cuda_alike() and torch_distributed_backend != "gloo"
