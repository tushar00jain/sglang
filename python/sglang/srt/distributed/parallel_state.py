...
# Backend filters for the ``split_group`` subgroup path.
#
...
def can_split_world() -> bool:
    ...
    ``init_distributed_environment`` (an embedding trainer, or ``multimodal_gen``)
    and therefore not in the shape split_group needs. Those worlds keep the
...
class GroupCoordinator:
    ...
    def __init__(
        ...
        # The world PG is device-qualified and created eagerly
        ...
        # "cuda:nccl" backend; the cpu subgroup additionally keeps "cpu:gloo"
        ...
        # comms (send/recv overlap), which a single split_group communicator
        # cannot give. That group is built members-only via new_group with
        # use_local_synchronization=True, which short-circuits non-members
        # before they would reach new_group's eager no-color-split path (not
        # every cuda backend implements one).
        # Its cpu group is split from the world PG like every other cpu group.
        #
        # Worlds that cannot be split (a plain gloo, mooncake, or NPU/XPU run,
        # or a world PG someone else initialized in a different shape) fall back
        # to the upstream new_group path.
        is_mooncake = "mooncake" in torch_distributed_backend
        use_split_group = can_split_world()
            ...
            if is_mooncake:
                ...
                if is_pipeline:
                    ...
                    device_group = torch.distributed.split_group(
                        split_ranks=[ranks],
                        backend=SPLIT_GROUP_DEVICE_BACKEND,
                        pg_options=get_torch_distributed_pg_options(group_name),
                        timeout=subgroup_timeout,
                    )
                ...
                #
                # The split necessarily keeps the parent's cuda backend
                ...
                # "cpu:gloo,cuda:nccl" config string.
                cpu_group = torch.distributed.split_group(
                    split_ranks=[ranks],
                    backend=SPLIT_GROUP_CPU_BACKEND,
...
def init_distributed_environment(
    ...
    if not torch.distributed.is_initialized():
        ...
        # Device-qualify the world backend and give it a cpu:gloo leg (see
        # SPLIT_GROUP_WORLD_BACKEND for why both are mandatory).
        world_backend = backend
        if world_backend in ("nccl", "cuda:nccl"):
            world_backend = SPLIT_GROUP_WORLD_BACKEND
...
def create_custom_parallel_group(
    ...
    Args:
        ...
        backend: The communication backend (default: "gloo"). Ignored on the
                 split_group path, where the group is split off the world PG and
                 so necessarily carries the parent's backends
                 (``cpu:gloo,cuda:nccl``).
    ...
    if can_split_world():
        # An eager new_group() here is fatal: non-member ranks take the
        # no-color-split path in _new_process_group_helper and call
        # perform_nocolor_split(), which not every cuda backend implements.
        ...
        for g_ranks in unique_groups:
            if covered & set(g_ranks):
                raise ValueError(
                    ...
                    f"the split_group path, but got overlapping splits: {unique_groups}"
                )
            covered |= set(g_ranks)
