...
from sglang.multimodal_gen.runtime.distributed.utils import (
    SPLIT_GROUP_CPU_BACKEND,
    SPLIT_GROUP_DEVICE_BACKEND,
    P2P_DEVICE_BACKEND,
    can_split_world,
)
...
class GroupCoordinator:
    ...
    def __init__(
        ...
        # An eager new_group() with asymmetric membership is fatal: the
        # *non-member* ranks take the no-color-split path in
        # _new_process_group_helper and call perform_nocolor_split(), which not
        # every cuda backend implements. The hazard is EAGER-vs-members-only,
        # not the requested backend -- _get_split_source inspects the parent's
        # CUDA backend, so backend="gloo" does not save you either.
        # So both subgroups are carved off the world PG with
        ...
        if can_split_world():
            ...
            device_group = torch.distributed.split_group(
                split_ranks=group_ranks, backend=SPLIT_GROUP_DEVICE_BACKEND
            )
            # A group carrying a `gloo` CPU backend, to allow direct
            # coordination between processes through the CPU. The split
            # necessarily keeps the parent's cuda backend alongside
            # cpu:gloo (see SPLIT_GROUP_CPU_BACKEND).
            with suppress_stdout():
                cpu_group = torch.distributed.split_group(
                    split_ranks=group_ranks, backend=SPLIT_GROUP_CPU_BACKEND
                )
...
class PipelineGroupCoordinator(GroupCoordinator):
    ...
    def __init__(
        ...
        # (isend/irecv/batch_isend_irecv), so they use the "nccl-lazy" backend
        # built members-only with use_local_synchronization=True: lazy per-peer
        # communicators let send/recv to different stages overlap, and the
        # members-only form short-circuits non-members before they would reach
        # the eager no-color-split path (not every cuda backend has one).
        #
        ...
        p2p_device_id = (
            ...
            if use_split_group and current_platform.is_cuda_alike()
            else None
        )
