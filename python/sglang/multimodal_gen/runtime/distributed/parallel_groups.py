...
def set_seq_parallel_pg_by_sp_groups(
    ...
    if can_split_world():
        # These groups have asymmetric membership, so an eager new_group()
        # would blow up on the non-member ranks with
        # "object has no attribute 'perform_nocolor_split'" whenever the
        # parent's cuda backend has no eager no-color-split path.
        # Carve them off the world PG instead: one
        ...
        def _split(rank_groups: list[list[int]]):
            ...
            pg = torch.distributed.split_group(
                split_ranks=rank_groups, backend=SPLIT_GROUP_DEVICE_BACKEND
            )
