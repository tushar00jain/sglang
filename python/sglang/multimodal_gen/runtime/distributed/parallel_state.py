...
def _sync_srt_world_group() -> None:
    ...
    Note: srt derives ``can_split_world()`` from the *live* default PG
    ...
    ``cpu:gloo,cuda:nccl`` above, srt's predicate flips to True for
...
def init_distributed_environment(
    ...
    if not torch.distributed.is_initialized():
        ...
        # Device-qualify the world backend as "cpu:gloo,cuda:nccl" (see
        # route_world_backend / SPLIT_GROUP_WORLD_BACKEND for why the
        # qualification is mandatory and why the cpu:gloo leg is needed for the
        # CPU subgroups).
...
def init_dit_group(
    ...
    if can_split_world():
        ...
        # express. split_group is collective over the
        ...
        _DIT = torch.distributed.split_group(
            split_ranks=[dit_ranks], backend=SPLIT_GROUP_DEVICE_BACKEND
        )
    else:
...
def init_vae_group(
    ...
    if can_split_world():
        ...
        # the sites that fails immediately on an eager new_group.
        _VAE = torch.distributed.split_group(
            split_ranks=[vae_ranks], backend=SPLIT_GROUP_DEVICE_BACKEND
        )
    else:
